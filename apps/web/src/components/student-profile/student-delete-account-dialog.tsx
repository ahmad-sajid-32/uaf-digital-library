...
import { STUDENT_ACCOUNT_DELETE_CONFIRMATION } from "@/lib/student-profile";

interface StudentDeleteAccountDialogProps {
  open: boolean;
...
export function StudentDeleteAccountDialog({
  open,
  onOpenChange,
  email,
  pending,
  ...
  onConfirm,
}: StudentDeleteAccountDialogProps): React.JSX.Element {
  const canConfirm =
    confirmationValue.trim() === STUDENT_ACCOUNT_DELETE_CONFIRMATION;
        ...
        <div className="space-y-5 px-6 py-6">
          ...
          <div className="rounded-3xl border border-destructive/20 bg-destructive/5 px-4 py-4 text-sm leading-6 text-foreground">
            ...
            <p className="mt-3 text-muted-foreground">
              Type <span className="font-semibold text-foreground">{STUDENT_ACCOUNT_DELETE_CONFIRMATION}</span>{" "}
              to confirm.
            </p>
          </div>
          ...
          <div className="space-y-2">
            <Input
              ...
              placeholder={STUDENT_ACCOUNT_DELETE_CONFIRMATION}
              disabled={pending}
